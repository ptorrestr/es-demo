...
@pytest.mark.parametrize("query", sorted(pathlib.Path("./queries").glob("q3*.json")))
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        print(
            "Top results for query: '{}', terms: '{}'".format(
                query, q["query"]["bool"]["must"][0]["match"]["description.spanish"]
            )
        )
        print("\t".join(["id", "score", "description", "group"]))
